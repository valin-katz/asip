...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    console.warn("AI_API_KEY is not set. Using local matching engine.");
  ...
  const topCatalog = topProjects.map((p, i) =>
    `${i+1}. [${p.project_id}] "${p.title}" — Mentor: ${p.guide || "N/A"}\n   Description: ${(p.description || "No description").slice(0, 250)}\n   End Users: ${(p.end_users || "Not specified").slice(0, 120)}\n   Students: ${(p.students || []).map(s => s.name).slice(0, 4).join(", ") || "N/A"}`
  ).join("\n\n");
...
- 220 active multidisciplinary engineering projects in the 2024–2026 cohort.

Tone & Strategy:
- Confident, authoritative, professional, and proud of Atria engineering students.
- Do NOT mention academic departments (such as ISE, CSE, ECE, Civil, Mechanical); focus on societal problems, domains, student engineers, and solutions.
- 1. DIRECT MATCH: If projects match, present them with project title, code, end users, student engineers, and faculty mentor.
- 2. ECOSYSTEM SYNERGY: If no single project covers 100%, combine 2–3 relevant projects to show how Atria's multidisciplinary ecosystem collaborates.
...
- Embed project cards using this exact structure:
  <div class="result-card" onclick="openModal('PROJECT_ID')">
    <div class="rc-meta"><span class="rc-id">PROJECT_ID</span></div>
...
${topCatalog}`;
  ...
  console.error("AI API failed, falling back to local response:", lastError);
  const fallbackText = generateLocalFallback(query);
  return res.status(200).json({ text: fallbackText });
}
